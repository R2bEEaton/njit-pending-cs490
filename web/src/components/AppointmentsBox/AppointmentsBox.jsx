...
const AppointmentsBox = ({ appointmentsJSON, appointmentsTasks }) => {
      ...
      updatedAppointmentTasks[category] = appointmentsTasks[category].map(
        (task) => {
          if (task.pomodoros >= 4) {
            ...
            totalTime =
              numLongBreaks * currentUser.longBreak +
              task.pomodoros * currentUser.pomodoro +
              (task.pomodoros - numLongBreaks) * currentUser.shortBreak
          } else {
            totalTime =
              currentUser.pomodoro * task.pomodoros +
              currentUser.shortBreak * task.pomodoros
          }
  ...
  function staggerIt() {
    let staggers = []
    let apptsCopy = scheduledItems.filter(({ allDay }) => {
      return !allDay
    })
