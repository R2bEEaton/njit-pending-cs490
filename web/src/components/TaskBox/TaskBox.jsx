...
const TaskBox = () => {
  ...
  const onDragEnd = (result) => {
    ...
    let tasksDataTemp = { ...tasksData }
    let taskItself =
      tasksDataTemp[result.source.droppableId][result.source.index]

    // Easter egg shhh
    let dndEasterEggTemp = [...dndEasterEgg]
    let swappedIds = [
      tasksDataTemp[result.source.droppableId][result.source.index]?.id,
      tasksDataTemp[result.destination.droppableId][result.destination.index]
        ?.id,
    ].sort()
    if (
      dndEasterEggTemp[dndEasterEggTemp.length - 1] !==
      JSON.stringify(swappedIds)
    )
      dndEasterEggTemp = []
    dndEasterEggTemp.push(JSON.stringify(swappedIds))
    updateDndEasterEgg(dndEasterEggTemp)
    if (dndEasterEggTemp.length % 3 === 0)
      toast.loading(
        `Ind${''.padStart(dndEasterEggTemp.length / 3, 'e')}cisive..?`,
        { duration: 5000 }
      )
    ...
    tasksDataTemp[result.destination.droppableId].splice(
